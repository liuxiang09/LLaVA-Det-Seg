...
def init_cocostuff(base_image_dir):
    ...
    cocostuff_classes = np.array(cocostuff_classes)     # 182个类别
    cocostuff_images = []

    cocostuff_labels = glob.glob(   # 118287个样本
...
class SemSegDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return (
            image_path,     # 图像路径
            image,          # 用于SAM的resize图像，应该是1024*1024
            image_clip,     # 用于CLIP的resize图像，应该是224*224
            conversations,  # 真值conversation
            masks,          # 真值masks shape： [n, h, w]，n对应的物体类别数 [h, w]是原大小
            label,          # [h, w],原始分割标签
            resize,         # [1024, 1024]
            questions,      # 问题
